...
def cubingchina_rank_name(url):
    ...
    person_list = page_selector.xpath('//div[@id="yw1"]/table/tbody/tr[position()<12]')
...
def WCA_rank_name(url):
    homepage = requests.get(url)
    page_selector = etree.HTML(homepage.text)
    person_name_list = page_selector.xpath('//div[@class="table-responsive"]/table/tbody/tr[position()<11]//td[@class="name"]//text()')
    return person_name_list
...
def cubingchina_get_WCA_ID(url):
    ...
    while (looptimes <= 10):
        ...
        person_WCA_ID = personalpage_selector.xpath('//div[@class="row"]/div[4]/span[2]//text()')
...
def WCA_get_WCA_ID(url):
    homepage = requests.get(url)
    page_selector = etree.HTML(homepage.text)
    person_WCA_ID_list = page_selector.xpath('//div[@class="table-responsive"]/table/tbody/tr[position()<11]//td[@class="wca-id"]//span[@class="wca-id"]/a//text()')
    return person_WCA_ID_list
...
def cubingchina_get_average(url , WCA_ID , event):
    ...
    while (looptimes < 10):
        ...
        single_person_name = re.findall('\((.*?)\)',cubingchina_rank_name(url)[looptimes][0],re.S)[0]
        print (single_person_name + ' , ' + single_WCA_ID)
        sheet.write(looptimes + 1, 0, single_person_name)
        # get average list
        WCA_ID_url = 'https://www.worldcubeassociation.org/persons/'+single_WCA_ID+'?event='+event
        homepage = requests.get(WCA_ID_url)
        page_selector = etree.HTML(homepage.text)
        raw_average_list = page_selector.xpath('//div[@id="results-by-event"]//tbody[@class="event-'+event+'"]')
        raw_average_list = raw_average_list[0].xpath('tr[@class="result"]/td[6]//text()')
        average_list = [0]*10
        average_num = 0
        ...
        num = 0
        while (num < 10):
            sheet.write(looptimes + 1, num + 1, float('%.2f'%average_list[num]))
            num = num + 1
        # 计算平均值和标准差
        ...
        while (average_num < (10 - average_list.count(0))):
            avg_list[average_num] = math.pow(average_list[average_num]-avg, 2)
            average_num = average_num + 1
        SD = math.sqrt(sum(avg_list) / (10-average_list.count(0)-1))
        print('Average: ' + str('%.3f'% avg) + '  ,  ' + 'S.D.： ' + str('%.3f'% SD))
...
def cubingchina_result_operate(competitors ,event):
    cubingchina_get_average(competitors + '?sort=' + event , cubingchina_get_WCA_ID(competitors + '?sort=' + event), event)


#final output , from WCA
def WCA_result_operate(comp_competitors , event):
    WCA_get_average(comp_competitors + '/psych-sheet/' + event,WCA_get_WCA_ID(comp_competitors + '/psych-sheet/' + event), event)
